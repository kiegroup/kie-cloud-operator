...
func TestRhdmEnvironmentWithKafkaJBPM(t *testing.T) {
	cr := &api.KieApp{
		...
		Spec: api.KieAppSpec{
			...
			Objects: api.KieAppObjects{
				Servers: []api.KieServerSet{
					{Kafka: createKafkaExtObject(), KafkaJbpm: createKafkaJbpmObject()},
				},
			},
		},
	}
	...
	assert.Equal(t, "true", getEnvVariable(env.Servers[0].DeploymentConfigs[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_KAFKA_EXT_ENABLED"))
	assert.Equal(t, "3", getEnvVariable(env.Servers[0].DeploymentConfigs[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_KAFKA_JBPM_ACKS"))
	assert.Equal(t, "localhost:9092", getEnvVariable(env.Servers[0].DeploymentConfigs[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_KAFKA_JBPM_BOOTSTRAP_SERVERS"))
	assert.Equal(t, "D12345678", getEnvVariable(env.Servers[0].DeploymentConfigs[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_KAFKA_JBPM_CLIENT_ID"))
	assert.Equal(t, "2000", getEnvVariable(env.Servers[0].DeploymentConfigs[0].Spec.Template.Spec.Containers[0], "KIE_SERVER_KAFKA_JBPM_MAX_BLOCK_MS"))
}

func checkJbpmKafkaEnvs(t *testing.T, env corev1.EnvVar) {
	switch e := env.Name; e {

	case "KIE_SERVER_KAFKA_JBPM_ACKS":
		assert.Equal(t, env.Value, "3")

	case "KIE_SERVER_KAFKA_JBPM_BOOTSTRAP_SERVERS":
		assert.Equal(t, env.Value, "localhost:9092")

	case "KIE_SERVER_KAFKA_JBPM_CLIENT_ID":
		assert.Equal(t, env.Value, "D12345678")

	case "KIE_SERVER_KAFKA_JBPM_MAX_BLOCK_MS":
		assert.Equal(t, env.Value, "2000")
	}
}

func createKafkaJbpmObject() *api.KafkaJBPMObject {
	kafkaJbpmObject := api.KafkaJBPMObject{
		Acks:             Pint(3),
		BootstrapServers: "localhost:9092",
		ClientID:         "D12345678",
		MaxBlockMs:       Pint32(2000),
	}
	return &kafkaJbpmObject
}
