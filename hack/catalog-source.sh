...
CSV=`cat deploy/catalog_resources/redhat/${VERSION}/businessautomation-operator.${VERSION}.clusterserviceversion.yaml | sed -e 's/^/          /' | sed '0,/ /{s/          /        - /}'`
CRD=`cat deploy/crds/kieapp.crd.yaml | sed -e 's/^/          /' | sed '0,/ /{s/          /        - /}'`
PKG=`cat deploy/catalog_resources/redhat/${VERSION}/businessautomation.${VERSION}.package.yaml | sed -e 's/^/          /' | sed '0,/ /{s/          /        - /}'`
